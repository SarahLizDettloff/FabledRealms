	...
	for i in get_slide_count():
		...
		if (collision.collider.name == "Player"):
			if player.attack_playing or player.is_attacking:
				hit_counter += 1
				if hit_counter >= 1:
					var enemey_defeat_location = $Blobset1AnimatedSprite.position
					blobset_bye()
...
func get_animation_for(direction: Vector2):
	if player.direction == "left":
		direction = right
		get_node( "Blobset1AnimatedSprite" ).set_flip_h(false)
	else:
		get_node( "Blobset1AnimatedSprite" ).set_flip_h(true)
		direction = left
...
func blobset_bye():
	...
	if timer > wait_time:
		...
		timer = 0	
	if not timer_start:
		get_node("Blobset1AnimatedSprite").hide()
